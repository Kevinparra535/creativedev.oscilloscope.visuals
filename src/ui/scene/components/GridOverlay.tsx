...
const GridOverlay = ({
  ...
  const { mainGridGeometry, subGridGeometry, centerCrosshairGeometry } =
    useMemo(() => {
      ...
      mainGeo.setAttribute(
        "position",
        new Float32BufferAttribute(mainPositions, 3)
      );

      const subGeo = new BufferGeometry();
      subGeo.setAttribute(
        "position",
        new Float32BufferAttribute(subPositions, 3)
      );

      const centerGeo = new BufferGeometry();
      centerGeo.setAttribute(
        "position",
        new Float32BufferAttribute(centerPositions, 3)
      );

      return {
        mainGridGeometry: mainGeo,
        subGridGeometry: subGeo,
        centerCrosshairGeometry: centerGeo,
      };
